...
url = "postgres://test@example.com:5432/db_one"
...
def test_parser_with_driver(parser):
    """
    The parser will exit if it receives a driver without a dest
    """
    with pytest.raises(SystemExit):
...
def test_parser_with_driver_and_destination(parser):
    """
    The parser will not exit if it receives a driver with a dest
    """
